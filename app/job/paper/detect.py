...
def find_paper(img):    
    ...
    if w<1000 or h<1000:
        print("没有图像或图像太小,请检查图片")
        return(2,None)
    #计算图像边缘一圈的像素平均值，若大于100则为A4纸   
    if (np.mean(img[:10, :w])+np.mean(img[h-10:h,:w])+np.mean(img[10:h-10,:10])+np.mean(img[10:h-10,w-10:w]))/4 > 200:
        print('扫描件')
        return 1,img
    # 转换为灰度图像
    ...
    #判断轮廓是否为矩形，且面积是否大于图片面积的3/4
    if len(max_contour) < 4 or cv2.contourArea(max_contour) < w * h /3:        
        print('找不到答题卡.')
        return 2,None
    # 使用多边形逼近找到轮廓的四个角点
    ...
    if (np.mean(dst[:10, :w])+np.mean(dst[h-10:h,:w])+np.mean(dst[10:h-10,:10])+np.mean(dst[10:h-10,w-10:w]))/4>100:
        print('找到答题卡')
        # 计算灰度图像的平均亮度值
