...
class teacher_add_all(FlaskForm):
    file = FileField(label="请先下载模板文件，填写好后上传", validators=[FileRequired('选择文件')],render_kw={'class': 'form-control'})
    submit = SubmitField("提交")

class students_add(FlaskForm):
    file = FileField(label="请先下载模板文件，填写好后上传", validators=[FileRequired('选择文件')],render_kw={'class': 'form-control'})
    submit3 = SubmitField("提交")
class photo_add(FlaskForm):
    #设置文件选择框
    file = FileField(label="请选择上传的照片，学生照片文件名为学生学号，教师照片文件名为教师姓名，若要上传个人照片，请在右上角个人菜单中操作", validators=[FileRequired('选择文件')], render_kw={'class': 'form-control'})
